...
//inicializando soporte para dispositivos touch
        ...
        $(".sidebar-nav .appt_container").draggable({connectToSortable: ".demo", helper: "clone", handle: ".drag", drag: function(e, t) {
                ...
                $(".demo .column").sortable({connectWith: ".column", cursorAt: {left: 20}, opacity: 0.8, helper: function(e, ui) {
        ...
        $(".sidebar-nav .box ,.cuadrante .box").draggable({connectToSortable: ".column", helper: "clone", handle: ".drag, .preview", drag: function(e, t) {
                ...
                $(".spaciador").resizable({
                    contaiment: $(e.target).parent().id,
                    helper: "ui-resizable-helper",
                    stop: function(e, ui) {
                        parent = $(e.target).parent();
                        total = parent.width();
                        wd = $(e.target).width();
                        md = 13 - Math.round(total/wd);
                        for (i = 1; i < 12; i++) {
                            $(e.target).removeClass("col-md-" + i);
                        }
                        $(e.target).addClass("col-md-"+md);
                        console.info("col-md-"+md);
                    }
                ...
                if (t.helper.hasClass('widget_column') || t.helper.hasClass('tab_widget')) {
                    ...
                    $(".column").sortable({connectWith: ".column", cursorAt: {left: 10}, opacity: 0.8, helper: function(e, ui) {
                            var widget = $(ui.context.outerHTML).attr("data-nombre");
                            var ico = $("#palette ul [data-nombre=" + widget + "] i").attr("class");
                            var boton = $('<i style="font-size: 35px;" class="' + ico + '"></i>');
                            boton.css({position: "absolute", left: 100, top: 30});
        ...
        $("[data-nombre='animacion']").draggable({
            helper: "clone", 
            handle: ".drag, .preview",
            drag: function(e, t) {
                //t.helper.width(400);
