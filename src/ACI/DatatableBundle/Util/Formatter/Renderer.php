...
namespace ACI\DatatableBundle\Util\Formatter;
        ...
        foreach ($data as $row_index => $row)
        {
            foreach ($row as $column_index => $column)
            {
                ...
                else
                {
                    ...
                    $view = 'GETDatatableBundle:Renderers:_default.html.twig';
                }
                try {
                    $id = $data[$row_index][count($data[$row_index])-1];
                } catch (Exception $exc) {
                    
                }

                $params = array_merge($params, array('dt_item' => $data[$row_index][$column_index], 'dt_id'=>$id));
